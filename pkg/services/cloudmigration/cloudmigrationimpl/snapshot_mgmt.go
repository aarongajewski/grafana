...
func (s *Service) buildSnapshot(
	...
	s.log.Debug("Build snapshot generated keys", "durationMs", time.Since(start).Milliseconds())
	...
	s.log.Debug("Build snapshot created snapshot writer", "durationMs", time.Since(start).Milliseconds())
	...
	s.log.Debug("Build snapshot fetched migration data", "durationMs", time.Since(start).Milliseconds())
	...
	case cloudmigration.ResourceStorageTypeDb:
		...
		s.log.Debug("Build snapshot wrote data partitions with database storage", "durationMs", time.Since(start).Milliseconds())
	...
	case cloudmigration.ResourceStorageTypeFs:
		...
		s.log.Debug("Build snapshot wrote data partitions with file system storage", "durationMs", time.Since(start).Milliseconds())
...
func (s *Service) uploadSnapshotWithFSStorage(ctx context.Context, session *cloudmigration.CloudMigrationSession, snapshotMeta *cloudmigration.CloudMigrationSnapshot, uploadUrl string) error {
	...
	for _, fileNames := range index.Items {
		for _, fileName := range fileNames {
			...
			s.log.Debug("Upload snapshot uploaded file", "fileName", fileName, "durationMs", time.Since(start).Milliseconds())
		}
	}
	uploadSpan.End()

	s.log.Debug("Upload snapshot uploaded all data files", "durationMs", time.Since(start).Milliseconds())
	...
	s.log.Debug("Upload snapshot uploaded index file", "durationMs", time.Since(start).Milliseconds())

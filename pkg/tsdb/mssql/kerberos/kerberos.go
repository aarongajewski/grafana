...
func getCredentialCacheFromLookup(lookupFile string, host string, port string, dbName string, user string) string {
	logger.Info("Reading Kerberos credential cache lookup", "lookupFile", lookupFile)
	content, err := os.ReadFile(filepath.Clean(lookupFile))
	if err != nil {
		logger.Error("Error reading Kerberos credential cache lookup", "lookupFile", lookupFile, "error", err)
		return ""
	}
	var lookups []KerberosLookup
	err = json.Unmarshal(content, &lookups)
	if err != nil {
		logger.Error("Error parsing Kerberos credential cache lookup", "lookupFile", lookupFile, "error", err)
		return ""
	}
	// find cache file
	for _, item := range lookups {
		...
		if item.Address == host+":"+port && item.DBName == dbName && item.User == user {
			logger.Info("Matched Kerberos credential cache lookup", "lookup", item)
			return item.CredentialCacheFilename
		}
	}
	logger.Error("No Kerberos credential cache match found", "address", host+":"+port)
	return ""
}
